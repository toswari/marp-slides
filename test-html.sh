#!/bin/bash
# MARP Slides HTML Unit Test
# Tests HTML export with proper HTML support

set -e

echo "=== MARP Slides HTML Unit Test ==="
echo ""

# Test 1: Check marp-cli is installed
echo "Test 1: Checking marp-cli installation..."
if command -v npx &> /dev/null; then
    MARP_VERSION=$(npx @marp-team/marp-cli --version 2>&1 | head -1)
    echo "  ✓ $MARP_VERSION"
else
    echo "  ✗ marp-cli not found"
    exit 1
fi

# Test 2: Create test file with CSS classes
echo ""
echo "Test 2: Creating test slide with CSS classes..."
cat > /tmp/test-marp.md << 'EOF'
---
marp: true
theme: default
paginate: true
title: Unit Test
style: |
  @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@400;600;700;800&family=Raleway:wght@100;200;300&display=swap');
  :root {
    --a: #ff6b1a; --bg: #000; --s: #080808; --b: #111;
    --m: #555; --t: #fff; --g: #22c55e; --body: #999; --label: #666;
  }
  section { background: var(--bg); color: var(--t); font-family: 'Raleway', sans-serif; padding: 56px 72px; }
  h1 { font-family: 'Outfit'; font-weight: 800; font-size: 3em; color: var(--t); }
  h2 { font-family: 'Raleway'; font-weight: 100; font-size: 1.3em; color: #888; }
  .card { background: var(--s); border: 1px solid var(--b); border-radius: 10px; padding: 18px; }
  .metric-label { font-size: 0.6em; color: var(--m); text-transform: uppercase; }
  .metric-value { font-size: 2em; font-weight: 800; color: var(--t); }
  .flex-row { display: flex; gap: 14px; }
  .flex-1 { flex: 1; }
---

# Unit Test

## Metrics Test

<div class="flex-row">
  <div class="flex-1 card">
    <div class="metric-label">Revenue</div>
    <div class="metric-value">$45,000</div>
  </div>
  <div class="flex-1 card">
    <div class="metric-label">Customers</div>
    <div class="metric-value">1,234</div>
  </div>
</div>
EOF
echo "  ✓ Test file created"

# Test 3: Export to HTML
echo ""
echo "Test 3: Exporting to HTML..."
npx @marp-team/marp-cli /tmp/test-marp.md --html --allow-local-files -o /tmp/test-marp.html 2>&1 | grep -E "INFO|WARN"
echo "  ✓ HTML generated"

# Test 4: Verify HTML content
echo ""
echo "Test 4: Verifying HTML content..."
if [ -f /tmp/test-marp.html ]; then
    if grep -q "Revenue" /tmp/test-marp.html; then
        echo "  ✓ 'Revenue' text found in HTML"
    else
        echo "  ✗ 'Revenue' text NOT found in HTML"
        exit 1
    fi
    if grep -q "45,000" /tmp/test-marp.html; then
        echo "  ✓ '45,000' text found in HTML"
    else
        echo "  ✗ '45,000' text NOT found in HTML"
        exit 1
    fi
    if grep -q ".card{" /tmp/test-marp.html; then
        echo "  ✓ CSS classes preserved in HTML"
    else
        echo "  ✗ CSS classes NOT found in HTML"
        exit 1
    fi
else
    echo "  ✗ HTML file not created"
    exit 1
fi

# Test 5: Check HTML file size
echo ""
echo "Test 5: Checking HTML file size..."
HTML_SIZE=$(ls -l /tmp/test-marp.html | awk '{print $5}')
if [ "$HTML_SIZE" -gt 5000 ]; then
    echo "  ✓ HTML size: $HTML_SIZE bytes (valid)"
else
    echo "  ✗ HTML size too small: $HTML_SIZE bytes"
    exit 1
fi

# Cleanup temp files
rm -f /tmp/test-marp.md /tmp/test-marp.html

# Convert all examples/*.md to HTML
echo ""
echo "=== Converting all examples to HTML ==="
ORIG_DIR="$(pwd)"
cd examples
COUNT=0
for mdfile in *.md; do
    if [ -f "$mdfile" ]; then
        htmlfile="${mdfile%.md}.html"
        echo "Converting: $mdfile => $htmlfile"
        npx @marp-team/marp-cli "$mdfile" --html --allow-local-files -o "$htmlfile" 2>&1 | grep -E "INFO" || true
        COUNT=$((COUNT + 1))
    fi
done
cd "$ORIG_DIR"
echo "  ✓ Converted $COUNT example files to HTML"

echo ""
echo "=== All Tests Passed ==="